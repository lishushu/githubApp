...
import com.lyc.gitassistant.network.base.ExceptionHandle
...
sealed class ResultState<out T> {
    companion object {
        ...
        fun <T> onAppError(error: NetException): ResultState<T> = Error(error)
    }
...
fun <T> MutableLiveData<ResultState<T>>.paresResult(result: BaseResponse<T>) {
    value = when {
        result.isSuccess() -> {
            ...
            ResultState.onAppError(NetException(result.getResponseCode(), result.getResponseMsg()))
        }
    }
}
